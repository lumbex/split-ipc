...
class S3FileUploadUtils {
  static params = {
    ALBUM_BUCKET_NAME: "vue-file-uploads-hub",
    ALBUM_BUCKET_LINK:
      "https://vue-file-uploads-hub.s3-us-west-2.amazonaws.com/",
    ...
    albums: {
      STAGING: "staging",
      YHARNN_PRODUCTS: "yharnnProducts"
    }
  };
  ...
  static async uploadFileToS3BucketAsync(
    ...
    let s3 = new AWS.S3({
      accessKeyId: S3FileUploadUtils.params.ACCESS_KEY,
      secretAccessKey: S3FileUploadUtils.params.SECRET_KEY
    });

    // Setting up S3 upload parameters
    const params = {
      Bucket: S3FileUploadUtils.params.ALBUM_BUCKET_NAME,
      Key: photoKey,
      Body: file,
      ACL: "public-read"
    };
  ...
  static getFileFinalUrl(oldKey, newName, targetAlbum) {
    return `${
      S3FileUploadUtils.params.ALBUM_BUCKET_LINK
  ...
  static async moveFileInS3Bucket(oldKey, newName, targetAlbum) {
    ...
    s3.copyObject({
      Bucket: S3FileUploadUtils.params.ALBUM_BUCKET_NAME,
      CopySource: `${S3FileUploadUtils.params.ALBUM_BUCKET_NAME}/${oldKey}`,
      Key: newKey
    })
      .promise()
      .then(() =>
...
export default S3FileUploadUtils;
